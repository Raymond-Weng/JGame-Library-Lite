...
最後，請在「確定所有Output準備工作已完成」的地方（例如建構子結尾、取得Game後等）使用Ready Checker(`jGame.main.ReadyChecker`)回報已完成，Output的準備代號(statCode)是`ReadyChecker.OUTPUT_READY`，實際製作如下：
```java
import jGame.core.Size;
